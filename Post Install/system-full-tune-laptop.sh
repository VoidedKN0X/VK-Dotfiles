...
#   - Kernel preset auto-detected (linux, linux-zen, linux-lts, linux-hardened, ...)
...
#      (REBOOT required, UKI/grub regenerated). cpufreq governor is left to
...
# === AUTO-DETECT installed kernel preset (for mkinitcpio) ===
# Picks the first installed linux* preset in mkinitcpio.d, preferring -zen if present.
detect_kernel_preset() {
  local preset=""
  local candidates=("linux-zen" "linux-hardened" "linux-lts" "linux")
  for c in "${candidates[@]}"; do
    if [[ -f "/etc/mkinitcpio.d/${c}.preset" ]]; then
      preset="$c"
      break
    fi
  done
...
KERNEL_PRESET="$(detect_kernel_preset)"
...
if [[ -f "$GRUB_FILE" ]]; then
  ...
  if [[ -n "$ADDED" ]]; then
    ...
    ok "kernel cmdline updated. Added:$ADDED"
      if [[ -n "$KERNEL_PRESET" ]] && [[ -f "/etc/mkinitcpio.d/${KERNEL_PRESET}.preset" ]] && command -v mkinitcpio >/dev/null; then
        if [[ -n "$UKI" && -f "$UKI" ]]; then
          cp -a "$UKI" "$BACKUP_DIR/$(basename "$UKI").$TS"
          chmod 600 "$BACKUP_DIR/$(basename "$UKI").$TS"
        fi
        if mkinitcpio -p "$KERNEL_PRESET" >/dev/null 2>&1; then
          ok "UKI regenerated for preset '$KERNEL_PRESET' (${UKI:-unknown})"
        else
          fail "mkinitcpio failed - UKI not regenerated; reboot with current params"
        fi
      else
        fail "no suitable mkinitcpio preset found (looked for linux, linux-zen, linux-lts, linux-hardened); cannot regenerate UKI"
      fi
  else
